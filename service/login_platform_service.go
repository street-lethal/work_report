...
func (s loginPlatformService) Login(id model.PlatformID) (*model.PlatformSession, error) {
	...
	path, cookies, err := s.req2(client, url, cookies)
	if err != nil {
		return nil, err
	}
	...
	pathID := pathParts[1]
	url = fmt.Sprintf("https://auth.levtech.jp/%s/sign_in", pathID)
	url, loginRes, cookies, err := s.req3(client, url, id, cookies)
	if err != nil {
		return nil, err
	}

	url, cookies, err = s.req4(client, loginRes.RedirectUri, cookies)
	if err != nil {
		return nil, err
	}

	url = "https://platform.levtech.jp/p/"
	url, cookies, err = s.req5(client, url, cookies)
	if err != nil {
		return nil, err
	}
