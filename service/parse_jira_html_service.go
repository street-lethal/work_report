...
type ParseJiraHTMLService interface {
	Parse(node *html.Node) map[int]model.Work
}

type parseJiraHTMLService struct {
	ParseHTMLService
}

func NewParseJiraHTMLService(hs ParseHTMLService) ParseJiraHTMLService {
	return &parseJiraHTMLService{hs}
}

func (s parseJiraHTMLService) Parse(node *html.Node) map[int]model.Work {
